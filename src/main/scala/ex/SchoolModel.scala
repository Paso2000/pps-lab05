...
object SchoolModel:
  def apply(): SchoolModule = BaseSchoolModel()

  private class BaseSchoolModel extends SchoolModule:
...
@main def examples(): Unit =
  val school = SchoolModel()   // Get the SchoolModule instance
  import school.*
  val empty = emptySchool           // Create an empty school
  println(empty.teachers)           // Nil()
  println(empty.courses)            // Nil()
  println(empty.hasTeacher("John")) // false
  println(empty.hasCourse("Math"))  // false
  val john = teacher("John")
  val math = course("Math")
  val italian = course("Italian")

  val school2 = empty.setTeacherToCourse(john, math)
